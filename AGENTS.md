...
Net Worth history is captured client-side only when a signed-in user opens a normal WorthIt section on the 1st or 15th of the month in `Asia/Manila`, beginning on `2026-09-15`. Each date uses one document at `users/{uid}/netWorthSnapshots/{YYYY-MM-DD}`. The current eligible-day snapshot is refreshed after relevant Spending or PlanIt source saves; after that Manila date ends, it becomes historical and locked. Missed dates remain missing; never backfill or fake a historical date.
